...
def get_col_name(columns, keywords, avoid=None):
    """Trova il nome colonna basandosi su keyword, ignorando termini specifici (es. Nascita)."""
    for col in columns:
...
def generate_pdf(df_atleta, nome, cognome):
    ...
    # Identificazione colonne per i dati
    ...
    # Header Grafico nel PDF
    pdf.set_fill_color(0, 80, 158) # Blu Aquatime
    ...
    pdf.cell(0, 10, f"Report Atleta: {nome.upper()} {cognome.upper()} | Data: {datetime.now().strftime('%d/%m/%Y')}", 0, 1, 'C')
    
    # Spazio e Tabella
    pdf.set_y(45)
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Arial", 'B', 11)
    pdf.cell(0, 10, "RIEPILOGO ULTIME SESSIONI", 0, 1, 'L')
    
    # Intestazioni Tabella
    pdf.set_font("Arial", 'B', 9)
    pdf.set_fill_color(230, 230, 230)
    w = [25, 45, 40, 20, 20, 25] # Larghezze colonne
    ...
    # 1. Distanza
    if c_km:
        axs[0, 0].plot(df_plot[c_data], df_plot[c_km], color='#00509E', marker='o', linewidth=2)
    ...
    res_text = f"TOTALI E MEDIE\n\nKm Totali: {km_tot:.1f}\nVelocità Media: {kmh_avg:.1f}\nMedia Calorie: {cal_avg:.0f}"
    ...
    pdf.cell(0, 10, "ANALISI PERFORMANCE", 0, 1, 'C')
    pdf.image(img_buf, x=10, y=35, w=190)
    plt.close(fig)
    
    return bytes(pdf.output()) # Converte bytearray in bytes per Streamlit
...
try:
    ...
    # --- RICERCA E REPORT ---
    st.divider()
    with st.expander("🔍 **RICERCA ATLETA E GENERA REPORT**", expanded=False):
        c_search1, c_search2 = st.columns(2)
        with c_search1: s_nome = st.text_input("Filtra Nome:", key="sn")
        with c_search2: s_cognome = st.text_input("Filtra Cognome:", key="sc")
        ...
        if (s_nome or s_cognome) and dati_per_ricerca:
            ...
            if not risultati.empty:
                ...
                mostrare = [c for c in risultati.columns if not any(x in c.upper() for x in parole_no)]
        ...
        with st.form("workout_form", clear_on_submit=True):
            ...
            if st.form_submit_button("🚀 Salva Dati"):
                if not n_ins or not c_ins or not s_ins:
                    st.error("Inserisci Nome, Cognome e Sede!")
                else:
                    client = get_gspread_client()
                    sheet = client.open_by_key(ID_FOGLIO).sheet1
                    nuova_riga = [f"{n_ins} {c_ins}", n_ins, c_ins, 0, "", d_ins.strftime("%d/%m/%Y"), sess_sel, prog_sel, liv_sel, v_ins, k_ins, cl_ins, s_ins, 0, 0, 0]
                    sheet.append_row(nuova_riga)
                    st.cache_data.clear()
                    st.success("Sessione registrata con successo!")
                    st.rerun()
